@extends('layout')
            ...
            <hr>
                  ...
                  <form action="{{ route('updateformation', $formation) }}" method='POST' enctype="multipart/form-data">
			    ...
			    <textarea class="form-control" name='titre' id="exampleFormControlTextarea1" rows="1">{{ $formation->contenue }}</textarea>{!! $errors->first('titre','<p class="text-danger">:message</p>') !!}
			    </div>


			    <div class="form-group">
			    <label for="exampleFormControlTextarea1">Déscripition</label>
			    <textarea class="form-control" name='contenue' id="exampleFormControlTextarea1" rows="2">{{ $formation->contenue }}</textarea>{!! $errors->first('contenue','<p class="text-danger">:message</p>') !!}
			    </div>
